...
TextureLoader::getSourceTextureFormat(pt::loaders::texture::TextureType type,
  ...
  //   return type == TextureType::sRGB ? MTL::PixelFormatRGBA8Unorm_sRGB :
...
Scene::AssetID TextureLoader::loadFromMemory(const uint8_t *data, uint32_t len,
   ...
   * We set the x-stride to four channels so our buffer can take any type of
   * input image. This assumes the image is in PNG format as that is usually the
...
Scene::AssetID TextureLoader::load(std::string_view name, TextureType type) {
  ...
  //     spec.channel_bytes() == 1 && spec.alpha_channel != -1;
  //
  // // All textures are loaded as 4bpc as Metal does not support RGB textures
  // with
  // // no alpha channel.
  // const size_t pixelStride = spec.channel_bytes() * 4;
  //
  // auto readBuffer = m_device->newBuffer(pixelStride * spec.width *
  // spec.height,
  //                                       MTL::ResourceStorageModeShared);
  // void *test = readBuffer->contents();
  // in->read_image(0, 0, 0, -1, spec.format, test,
  // OIIO::stride_t(pixelStride)); in->close();
  //
  // /*
  ...
  // srcTexture->replaceRegion(MTL::Region(0, 0, 0, spec.width, spec.height, 1),
  // 0,
  //                           readBuffer->contents(), pixelStride *
  //                           spec.width);
  //
  // /*
  ...
  // auto threadgroups =
  //     MTL::Size((spec.width + threadsPerThreadgroup.width - 1) /
  //                   threadsPerThreadgroup.width,
  //               (spec.height + threadsPerThreadgroup.height - 1) /
