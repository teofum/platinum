 ...
 */
namespace bsdf {
  ShadingContext::ShadingContext(device const pt::BSDF& mat, float2 uv, device Texture* textures) {
  ...
  Eval BSDF::evalTransparentDielectric(float3 wo, float3 wi, float3 wm, float fresnel_ss, float ior) {
    const bool thin = m_ctx.flags & pt::BSDF::Material_ThinDielectric;
  ...
  Sample BSDF::sampleOpaqueDielectric(float3 wo, float3 r) {
    ...
    if (r.z < blendingFactor.x) {
      ...
      if (m_ctx.flags & pt::BSDF::Material_Emissive) flags |= Sample_Emitted;
      return {
        .flags  = flags,
        .wi     = wi,
        .f      = m_ctx.albedo * cDiffuse,
