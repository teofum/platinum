 ...
 */
namespace bsdf {
ShadingContext::ShadingContext(device const MaterialGPU &mat, float2 uv,
                               float3x3 idt, device Texture *textures) {
  ...
  if (mat.baseTextureId >= 0)
    albedo = textures[mat.baseTextureId].tex.sample(s, uv).rgb;
  if (mat.emissionTextureId >= 0)
    emission *= textures[mat.emissionTextureId].tex.sample(s, uv).rgb;
  if (mat.transmissionTextureId >= 0)
    transmission = textures[mat.transmissionTextureId].tex.sample(s, uv).r;
  if (mat.clearcoatTextureId >= 0)
    clearcoat = textures[mat.clearcoatTextureId].tex.sample(s, uv).r;
...
/*
 * Numerical fit for average conductor fresnel supporting an edge tint
 * parameter.
 ...
 * We don't use the complex equations for conductors, as they're meaningless in
 * RGB rendering
 */
float fresnel(float cosTheta, float ior) {
  ...
  const auto parallel =
      (ior * cosTheta - cosTheta_t) / (ior * cosTheta + cosTheta_t);
  const auto perpendicular =
      (cosTheta - ior * cosTheta_t) / (cosTheta + ior * cosTheta_t);
  return (parallel * parallel + perpendicular * perpendicular) * 0.5f;
}

/*
 * Numerical fit for average dielectric fresnel.
 * Reference: Revisiting Physically Based Shading at Imageworks, C. Kulla & A.
 * Conty 2017
 */
float avgDielectricFresnelFit(float ior) {
  return ior >= 1.0f ? (ior - 1.0f) / (4.08567f + 1.00071f * ior)
                     : 0.997118f + 0.1014f * ior - 0.965241 * ior * ior -
                           0.130607 * ior * ior * ior;
}

/*
 * Trowbridge-Reitz GGX microfacet distribution implementation
 * Simple wrapper around all the GGX sampling functions, supports anisotropy.
 * Works with all directions in tangent space (Z-up, aligned to surface normal)
 */
...
inline float GGX::mdf(float3 w) const {
  ...
  float k = 1.0f / cos2Theta *
            (w.x * w.x / (m_alpha.x * m_alpha.x) +
             w.y * w.y / (m_alpha.y * m_alpha.y));
...
inline float GGX::g1(float3 w) const { return 1.0f / (1.0f + lambda(w)); }
...
inline float3 GGX::sampleVmdf(float3 w, float2 u) const {
  ...
  const auto b = (wh.z < 0.9999f) ? normalize(cross(float3(0.0, 0.0, 1.0), wh))
                                  : float3(1.0, 0.0, 0.0);
  ...
  return normalize(
      float3(m_alpha.x * nh.x, m_alpha.y * nh.y, max(1e-6f, nh.z)));
}

__attribute__((always_inline)) float
GGX::singleScatterBRDF(float3 wo, float3 wi, float3 wm) const {
  return mdf(wm) * g(wo, wi) / (4 * abs(wo.z) * abs(wi.z));
}

__attribute__((always_inline)) float GGX::pdf(float3 wo, float3 wm) const {
  return vmdf(wo, wm) / (4.0f * abs(dot(wo, wm)));
}

// Surfaces with a very small roughness value are considered perfect specular,
// this helps deal with the numerical instability at such values and makes
...
inline float GGX::lambda(float3 w) const {
  ...
  if (m_alpha.x != m_alpha.y) {
    alpha2 = alpha2 * w.x * w.x + m_alpha.y * m_alpha.y * w.y * w.y;
  }

  return (sqrt(1.0f + alpha2 / cos2Theta) - 1.0f) * 0.5f;
}

/*
 * Principled BSDF implementation.
 * Loosely based on a subset of the Enterprise PBR spec and Blender's Principled
 * BSDF.
 */
BSDF::BSDF(thread ShadingContext &ctx, constant Constants &constants,
           constant Luts &luts)
    : m_ctx(ctx), m_ggx(ctx.roughness, ctx.anisotropy),
      m_ggxCoat(ctx.clearcoatRoughness), m_constants(constants), m_luts(luts) {}

/*
 * Evaluate the BSDF, given outgoing and incident light directions. Blends all
 * BSDF lobes based on material parameters and the PDF for each.
 */
Eval BSDF::eval(float3 wo, float3 wi) {
  ...
  if (metallic > 0.0)
    result += evalMetallic(wo, wi) * metallic;
  if (transparent > 0.0)
    result += evalTransparentDielectric(wo, wi) * transparent;
  if (opaque > 0.0)
    result += evalOpaqueDielectric(wo, wi) * opaque; // TODO glossy
...
Sample BSDF::sample(float3 wo, float4 r, float2 rc) {
  ...
  if (pClearcoat > 0.0f) {
    const float3 wmCoat =
        m_ggxCoat.isSmooth() ? float3(0, 0, 1) : m_ggxCoat.sampleVmdf(wo, rc);
    pClearcoat *= fresnel(abs(dot(wo, wmCoat)), m_clearcoatIor);
  }

  const float pMetallic = pClearcoat + (1.0f - pClearcoat) * m;
  const float pTransparent =
      pClearcoat + (1.0f - pClearcoat) * (m + (1.0f - m) * t);

  if (r.w < pClearcoat)
    return sampleClearcoat(wo, r.xyz);
  if (r.w < pMetallic)
    return sampleMetallic(wo, r.xyz);
  if (r.w < pTransparent)
    return sampleTransparentDielectric(wo, r.xyz);
 ...
 * E. Turquin's method
 * (https://blog.selfshadow.com/publications/turquin/ms_comp_final.pdf) This
 * method is not reciprocal, but I couldn't get the K&C method working for a
 * transparent BSDF.
 * TODO: revisit this later
 */
__attribute__((always_inline)) float
BSDF::transparentMultiscatter(float3 wo, float3 wi, float ior) {
  ...
  if (ior < 1.0f) {
    ...
    const auto E_wo =
        m_luts.ETransOut.sample(s, float3(abs(wo.z), m_ctx.roughness, iorParam))
            .r;
    ...
    const auto E_wo =
        m_luts.ETransIn.sample(s, float3(abs(wo.z), m_ctx.roughness, iorParam))
            .r;
...
/*
 * Calculate the attenuation factor for the underlying diffuse BRDF in a
 * "glossy" (diffuse + GGX) lobe. Assumes the microfacet BRDF is compensated for
 * multiple scattering. Reference: Enterprise PBR spec
 */
__attribute__((always_inline)) float BSDF::diffuseFactor(float3 wo, float3 wi) {
  // IOR parametrization optimized for the common IOR range 1 < eta < 2
  // See https://www.desmos.com/calculator/1pkvgrisbx for details.
  ...
  constexpr sampler s(address::clamp_to_edge, filter::linear);
  const auto E_ms_wo =
      m_luts.EMs.sample(s, float3(wo.z, m_ctx.roughness, iorParam)).r;
  const auto E_ms_wi =
      m_luts.EMs.sample(s, float3(wi.z, m_ctx.roughness, iorParam)).r;
  const auto E_ms_avg =
      m_luts.EavgMs.sample(s, float2(iorParam, m_ctx.roughness)).r;

  return (1.0f - E_ms_wo) * (1.0f - E_ms_wi) / (M_PI_F * (1.0f - E_ms_avg));
}

/*
 * Blending factor for opaque dielectrics, accounting for a multiple scattering
 * dielectric BRDF. Returns the blending weight of the dielectric component.
 */
__attribute__((always_inline)) float BSDF::opaqueDielectricFactor(float3 wo,
                                                                  float F_avg) {
  // IOR parametrization optimized for the common IOR range 1 < eta < 2
  // See https://www.desmos.com/calculator/1pkvgrisbx for details.
  const auto iorParam = (m_ctx.ior - 1.0f) / m_ctx.ior;
 ...
 * Internal version, lets us skip the microfacet normal calculation if we
 * already have it (i.e. if we just sampled the VMDF)
 */
__attribute__((always_inline)) Eval BSDF::evalMetallic(float3 wo, float3 wi,
                                                       float3 wm) {
 ...
 * Internal version, lets us skip the microfacet normal and fresnel calculations
 * if we already have them (i.e. after sampling the BSDF)
 */
__attribute__((always_inline)) Eval BSDF::evalTransparentDielectric(
    float3 wo, float3 wi, float3 wm, float fresnel_ss, float ior) {
  ...
  if (isReflection) {
    ...
    if (thin) {
      ...
      btdf_ss = m_ggx.mdf(wm) * m_ggx.g(wo, wi) *
...
Eval BSDF::evalTransparentDielectric(float3 wo, float3 wi) {
  ...
  const bool thin = m_ctx.flags & MaterialGPU::Material_ThinDielectric;
  const float ior =
      (!thin && wo.z < 0.0f && wi.z < 0.0f) ? 1.0f / m_ctx.ior : m_ctx.ior;

  float3 wm = ior * wi + wo;
  if (wi.z == 0 || wo.z == 0 || wm.z == 0)
    return {};

  wm = normalize(wm * sign(wm.z));
  if (dot(wi, wm) * wi.z < 0.0f || dot(wo, wm) * wo.z < 0.0f)
    return {};
...
Eval BSDF::evalOpaqueDielectric(float3 wo, float3 wi) {
  ...
  return {
      ...
      .pdf = m_ggx.pdf(wo, wm) * blendingFactor +
             diffusePdf * (1.0f - blendingFactor),
  };
}
 ...
 * Get an incident light direction by importance sampling the BRDF, and evaluate
 * it
 */
Sample BSDF::sampleMetallic(float3 wo, float3 r) {
  ...
  // Sample the microfacet normal, get incident light direction and evaluate the
  // BRDF
 ...
 * Get an incident light direction by importance sampling the BSDF, and evaluate
 * it
 */
...
Sample BSDF::sampleOpaqueDielectric(float3 wo, float3 r) {
  ...
  if (r.z < blendingFactor) {
    ...
    if (m_ctx.flags & MaterialGPU::Material_Emissive)
      flags |= Sample_Emitted;
    return {
        .flags = flags,
        .wi = wi,
        .f = m_ctx.albedo * cDiffuse,
