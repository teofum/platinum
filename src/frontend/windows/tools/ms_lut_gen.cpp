...
void MultiscatterLutGenerator::init(MTL::Device *device,
...
void MultiscatterLutGenerator::render() {
  ...
  ImGui::BeginChild("RenderView", {256, 256}, ImGuiChildFlags_Borders,
  ...
  if (m_accumulator[0] != nullptr) {
    if (dim == 3) {
      ...
      benc->copyFromTexture(m_accumulator[0], 0, 0,
                            MTL::Origin(0, 0, m_viewSliceIdx),
                            MTL::Size(size.x, size.y, 1), m_viewSlice, 0, 0,
                            MTL::Origin(0, 0, 0));
      benc->endEncoding();
      cmd->commit();
      cmd->waitUntilCompleted();

      ImGui::Image((ImTextureID)m_viewSlice, {256, 256});
    } else {
      ImGui::Image((ImTextureID)m_accumulator[0], {256, 256});
    }
  }
  ...
  auto progressStr = m_frameIdx == m_accumulateFrames ? "Done!"
                     : m_frameIdx == 0                ? "Ready"
                                       : std::format("{} / {}", m_frameIdx,
                                                     m_accumulateFrames);
  ...
  if (widgets::combo("##LUTOption", m_lutOptions[m_selectedLut].displayName)) {
    for (uint32_t i = 0; i < m_lutOptions.size(); i++) {
      ...
      if (widgets::comboItem(m_lutOptions[i].displayName, isSelected))
        m_selectedLut = i;
  ...
  if (widgets::combo("LUT Size", std::format("{}px", m_lutSize).c_str())) {
    for (uint32_t size = 16; size <= 1024; size <<= 1) {
      ...
      if (widgets::comboItem(label.c_str(), isSelected))
        m_lutSize = size;
...
void MultiscatterLutGenerator::frame() {
  ...
  while (time < 1000 / 120 && m_frameIdx < m_accumulateFrames) {
    ...
    auto threadgroups = MTL::Size(
        (m_lutSize + threadsPerThreadgroup.width - 1) /
            threadsPerThreadgroup.width,
        ((dim > 1 ? m_lutSize : 1) + threadsPerThreadgroup.height - 1) /
            threadsPerThreadgroup.height,
        ((dim > 2 ? m_lutSize : 1) + threadsPerThreadgroup.depth - 1) /
    ...
    time =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - frameStart)
            .count();
...
void MultiscatterLutGenerator::generate() {
   ...
   */
  if (m_accumulator[0] != nullptr)
    m_accumulator[0]->release();
  if (m_accumulator[1] != nullptr)
    m_accumulator[1]->release();
  if (m_randomSource != nullptr)
    m_randomSource->release();
  if (m_viewSlice != nullptr)
    m_viewSlice->release();
  ...
  for (size_t i = 0; i < k; i++)
    random[i] = rand() % (1024 * 1024);
  auto region = MTL::Region(0, 0, 0, m_lutSize, dim > 1 ? m_lutSize : 1,
                            dim > 2 ? m_lutSize : 1);
  m_randomSource->replaceRegion(region, 0, random.data(),
   ...
   * This is just an internal dev tool, doesn't need to be very fast so we can
   * just regen the PSO on every render if we need a different one
   */
  if (m_pso != nullptr)
    m_pso->release();
  ...
  if (!lib) {
    std::println("ms_lut_gen: Failed to load shader library: {}",
                 error == nullptr
                     ? "Unknown error"
  ...
  auto desc = metal_utils::makeComputePipelineDescriptor({
      .function =
          metal_utils::getFunction(lib, m_lutOptions[m_selectedLut].kernelName),
      .threadGroupSizeIsMultipleOfExecutionWidth = true,
  });

  m_pso = m_device->newComputePipelineState(desc, MTL::PipelineOptionNone,
                                            nullptr, &error);
  if (!m_pso) {
    std::println("ms_lut_gen: Failed to create pipeline: {}",
                 error->localizedDescription()->utf8String());
    assert(false);
  }
