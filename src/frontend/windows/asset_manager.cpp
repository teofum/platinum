...
void AssetManager::renderAssetsPanel() {
  ...
  if (showChild) {
    ...
    ImGuiMultiSelectFlags msFlags =
      ImGuiMultiSelectFlags_ClearOnEscape | ImGuiMultiSelectFlags_ClearOnClickVoid | ImGuiMultiSelectFlags_BoxSelect2d;
      ...
      for (uint32_t rowIdx = clipper.DisplayStart; rowIdx < clipper.DisplayEnd; rowIdx++) {
        ...
        for (uint32_t itemIdx = rowBegin; itemIdx < rowEnd; itemIdx++) {
          ...
          ImGui::PushStyleColor(
            ...
            theme::imguiRGBA(mix(theme->bgObject, theme->primary, float3(0.5))));
