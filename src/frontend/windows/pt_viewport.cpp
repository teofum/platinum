...
RenderViewport::RenderViewport(Store &store, float &dpiScaling,
                               bool *open) noexcept
  ...
  if (open && ImGui::IsItemVisible()) {
    ...
    if (ImGui::BeginCombo("##CameraSelect", label.c_str())) {
      for (const auto &camera : cameras) {
        auto name = std::format("{}##Camera_{}", camera.node.name(),
                                uint32_t(camera.node.id()));
        const bool isSelected = camera.node.id() == m_cameraNodeId;
        if (widgets::comboItem(name.c_str(), isSelected))
          m_cameraNodeId = camera.node.id();
    ...
    if (ImGui::BeginChild("RenderView", size, ImGuiChildFlags_None,
    ...
    auto progressStr = accumulated == total ? "Done!"
                       : accumulated == 0
                           ? "Ready"
    ...
    if (accumulated == total)
      m_store.setRendering(false);

    auto time =
        std::format("{:.3f}s", (float)m_renderer->renderTime() / 1000.0f);
    auto textWidth = ImGui::CalcTextSize(time.c_str()).x;
    ImGui::SameLine(ImGui::GetContentRegionAvail().x +
                    ImGui::GetStyle().ItemSpacing.x - textWidth);
...
void RenderViewport::renderSettingsWindow(
    const std::vector<Scene::CameraInstance> &cameras,
    const std::string &label) {
  ImGui::Begin("Render Settings");

  ImGui::BeginDisabled(
      !(m_renderer->status() & renderer_pt::Renderer::Status_Ready));
  ...
  ImGui::InputFloat2(
      "##OutputSize",
      (float *)(m_useViewportSizeForRender ? &scaledSize : &m_nextRenderSize),
      "%.0fpx");
  ImGui::EndDisabled();
  ...
  if (widgets::combo("Render kernel", kernelNames[selectedKernel].c_str())) {
    if (widgets::comboItem(kernelNames[0].c_str(), selectedKernel == 0))
      m_renderer->selectKernel(
          uint32_t(renderer_pt::Renderer::Integrators::Simple));
    if (selectedKernel == 0)
      ImGui::SetItemDefaultFocus();

    if (widgets::comboItem(kernelNames[1].c_str(), selectedKernel == 1))
      m_renderer->selectKernel(
          uint32_t(renderer_pt::Renderer::Integrators::MIS));
    if (selectedKernel == 1)
      ImGui::SetItemDefaultFocus();
  ...
  ImGui::CheckboxFlags("Multiscatter GGX", &m_renderFlags,
                       shaders_pt::RendererFlags_MultiscatterGGX);
  ImGui::CheckboxFlags("GMoN Estimator", &m_renderFlags,
                       shaders_pt::RendererFlags_GMoN);
  ...
  ImGui::BeginDisabled(
      !(m_renderer->status() & renderer_pt::Renderer::Status_Ready));
  if (widgets::combo("Working space",
                     m_colorspaces.at(m_workingSpace).c_str())) {
    for (const auto &[cs, name] : m_colorspaces) {
      if (widgets::comboItem(name.c_str(), m_workingSpace == cs))
        m_workingSpace = cs;
      if (m_workingSpace == cs)
        ImGui::SetItemDefaultFocus();
    }
    ImGui::EndCombo();
  }
  ...
  if (widgets::combo("Output space", m_colorspaces.at(m_outputSpace).c_str())) {
    for (const auto &[cs, name] : m_colorspaces) {
      if (widgets::comboItem(name.c_str(), m_outputSpace == cs))
        m_outputSpace = cs;
      if (m_outputSpace == cs)
        ImGui::SetItemDefaultFocus();
    }
...
void RenderViewport::renderPostprocessSettings() {
  ...
  for (const auto &options : m_renderer->postProcessOptions()) {
    ...
    case postprocess::PostProcessPass::Type::Exposure: {
      ...
      widgets::dragFloat("Exposure", &exposureOptions.exposure, 0.1f, -5.0f,
                         5.0f, "%.1f EV");
      break;
    }
    case postprocess::PostProcessPass::Type::ContrastSaturation: {
      ...
      widgets::dragFloat("Contrast", &csOptions.contrast, 1.0f, -100.0f, 100.0f,
                         "%.0f");
      widgets::dragFloat("Saturation", &csOptions.saturation, 1.0f, -100.0f,
                         100.0f, "%.0f");
      break;
    }
    case postprocess::PostProcessPass::Type::ToneCurve: {
      ...
      widgets::dragFloat("Blacks", &toneCurveOptions.blacks, 1.0f, -100.0f,
                         100.0f, "%.0f");
      widgets::dragFloat("Shadows", &toneCurveOptions.shadows, 1.0f, -100.0f,
                         100.0f, "%.0f");
      widgets::dragFloat("Highlights", &toneCurveOptions.highlights, 1.0f,
                         -100.0f, 100.0f, "%.0f");
      widgets::dragFloat("Whites", &toneCurveOptions.whites, 1.0f, -100.0f,
                         100.0f, "%.0f");
      break;
    }
    case postprocess::PostProcessPass::Type::Vignette: {
      ...
      widgets::dragFloat("Amount", &vignetteOptions.amount, 0.1f, -5.0f, 5.0f,
                         "%.1f EV");
      widgets::dragFloat("Midpoint", &vignetteOptions.midpoint, 1.0f, -100.0f,
                         100.0f, "%.0f");
      widgets::dragFloat("Feather", &vignetteOptions.feather, 1.0f, 0.0f,
                         100.0f, "%.0f");
      widgets::dragFloat("Power", &vignetteOptions.power, 1.0f, 0.0f, 100.0f,
                         "%.0f");
      widgets::dragFloat("Roundness", &vignetteOptions.roundness, 1.0f, 0.0f,
                         100.0f, "%.0f");
      break;
    }
    case postprocess::PostProcessPass::Type::ChromaticAberration: {
      ...
      widgets::dragFloat("Amount", &caOptions.amount, 1.0f, -100.0f, 100.0f,
                         "%.0f");
      widgets::dragFloat("Green Shift", &caOptions.greenShift, 1.0f, -100.0f,
                         100.0f, "%.0f");
      break;
    }
    default:
  ...
  if (widgets::combo("Tonemap",
                     m_tonemappers.at(tonemapOptions.tonemapper).c_str())) {
    for (const auto &[tonemapper, name] : m_tonemappers) {
      bool isSelected = tonemapOptions.tonemapper == tonemapper;
      if (widgets::comboItem(name.c_str(), isSelected))
        tonemapOptions.tonemapper = tonemapper;
  ...
  case postprocess::Tonemapper::AgX: {
    ...
    if (widgets::button("None", {buttonWidth, 0}))
      look = postprocess::agx::looks::none;
    ImGui::SameLine();
    if (widgets::button("Golden", {buttonWidth, 0}))
      look = postprocess::agx::looks::golden;
    ImGui::SameLine();
    if (widgets::button("Punchy", {buttonWidth, 0}))
      look = postprocess::agx::looks::punchy;

    ImGui::Spacing();

    widgets::dragVec3("Offset", (float *)&look.offset, 0.01f, -10.0f, 10.0f,
                      "%.2f");
    widgets::dragVec3("Slope", (float *)&look.slope, 0.01f, -5.0f, 5.0f,
                      "%.2f");
    widgets::dragVec3("Power", (float *)&look.power, 0.01f, 0.0f, 5.0f, "%.2f");
    widgets::dragFloat("Saturation", &look.saturation, 0.01f, 0.0f, 3.0f,
                       "%.2f");
  ...
  case postprocess::Tonemapper::KhronosPBR: {
    ...
    widgets::dragFloat("Threshold", &options.compressionStart, 0.01f, 0.2f,
                       1.0f, "%.2f");
    widgets::dragFloat("Desaturation", &options.desaturation, 0.01f, 0.0f, 1.0f,
                       "%.2f");
  ...
  case postprocess::Tonemapper::flim: {
    ...
    if (widgets::button("Default", {buttonWidth, 0}))
      options = postprocess::flim::presets::flim;
    ImGui::SameLine();
    if (widgets::button("Silver", {buttonWidth, 0}))
      options = postprocess::flim::presets::silver;

    ImGui::Spacing();

    widgets::dragFloat("Pre-exposure", &options.preExposure, 0.1f, -10.0f,
                       10.0f, "%.1f EV");

    ImGui::Spacing();

    widgets::dragFloat("Min EV", &options.sigmoidLog2Min, 0.1f, -20.0f, 50.0f,
                       "%.1f EV");
    widgets::dragFloat("Max EV", &options.sigmoidLog2Max, 0.1f, -20.0f, 50.0f,
                       "%.1f EV");
    widgets::dragVec2("Toe", (float *)&options.sigmoidToe, 0.001f, 0.0f, 1.0f);
    widgets::dragVec2("Shoulder", (float *)&options.sigmoidShoulder, 0.001f,
                      0.0f, 1.0f);

    ImGui::Spacing();

    widgets::color("Pre filter", (float *)&options.preFormationFilter);
    widgets::dragFloat("Pre strength", &options.preFormationFilterStrength,
                       0.001f, 0.0f, 1.0f);
    widgets::color("Post filter", (float *)&options.postFormationFilter);
    widgets::dragFloat("Post strength", &options.postFormationFilterStrength,
                       0.001f, 0.0f, 1.0f);

    ImGui::Spacing();

    widgets::dragFloat("Neg. Exposure", &options.negativeExposure, 0.1f, -10.0f,
                       10.0f, "%.1f EV");
    widgets::dragFloat("Neg. Density", &options.negativeDensity, 0.5f, 0.0f,
                       100.0f, "%.1f");
    widgets::dragFloat("Print Exposure", &options.printExposure, 0.1f, -10.0f,
                       10.0f, "%.1f EV");
    widgets::dragFloat("Print Density", &options.printDensity, 0.5f, 0.0f,
                       100.0f, "%.1f");
    ...
    widgets::dragFloat("Black point", &options.blackPoint, 0.01f, 0.0f, 1.0f,
                       "%.2f");
    ImGui::EndDisabled();
    widgets::dragFloat("Midtone Sat.", &options.midtoneSaturation, 0.01f, 0.0f,
                       10.0f, "%.2f");
  ...
  float3 gain =
      1.0 + gainColor + tonemapOptions.postTonemap.highlightOffset * 0.01;

  float3 midGray =
      0.5 + gammaColor + tonemapOptions.postTonemap.midtoneOffset * 0.01;
  ...
  if (ImPlot::BeginPlot("##GradingCurves", {-1, 150})) {
    auto flags = ImPlotAxisFlags_Lock | ImPlotAxisFlags_NoTickLabels |
                 ImPlotAxisFlags_NoHighlight | ImPlotAxisFlags_NoLabel;
  ...
  widgets::color("Highlights",
                 (float *)&tonemapOptions.postTonemap.highlightColor);
  widgets::dragFloat("Shadows", &tonemapOptions.postTonemap.shadowOffset, 1.0f,
                     -100.0f, 100.0f, "%.0f");
  widgets::dragFloat("Midtones", &tonemapOptions.postTonemap.midtoneOffset,
                     1.0f, -100.0f, 100.0f, "%.0f");
  widgets::dragFloat("Highlights", &tonemapOptions.postTonemap.highlightOffset,
                     1.0f, -100.0f, 100.0f, "%.0f");
}

void RenderViewport::startRender() {
  if (canRender()) {
    m_renderSize = m_useViewportSizeForRender ? m_viewportSize * m_dpiScaling
                                              : m_nextRenderSize;
    m_renderer->startRender(
        m_cameraNodeId.value(), m_renderSize, (uint32_t)m_nextRenderSampleCount,
        m_gmonBuckets, color::getColorspace(m_workingSpace), m_renderFlags);
    m_store.setRendering(true);
  }
}

bool RenderViewport::canRender() const {
  return m_cameraNodeId &&
         (m_renderer->status() & renderer_pt::Renderer::Status_Ready);
}
...
void RenderViewport::updateScrollAndZoomState() {
  if (length_squared(m_scrollSpeed) < m_scrollStop * m_scrollStop) {
    ...
    m_scrollSpeed -=
        normalize(m_scrollSpeed) * min(m_scrollFriction, length(m_scrollSpeed));
  ...
  m_offset = (m_offset - m_zoomCenter) * (m_zoomFactor / lastZoomFactor) +
             m_zoomCenter;
