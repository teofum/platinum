...
void Environment::rebuildAliasTable(MTL::Device* device, MTL::Texture* texture) {
  ...
  for (auto& pixel: importance) pixel *= scale;
  ...
  for (uint64_t i = 0; i < n; i++) {
    if (importance[i] < 1.0) small.push_back(i);
  ...
  while (!small.empty() && !large.empty()) {
    ...
    importance[g] = (importance[g] + importance[l]) - 1.0;
    if (importance[g] < 1.0) small.push_back(g);
