...
public:
  ...
  SharedPtr(
    ...
    typename std::enable_if_t<std::is_convertible_v<_OtherClass*, _Class*>>* = nullptr
  ) noexcept;
  ...
  /**
   * Implicit cast to pointee.
   */
  operator const _Class*() const;
...
template<class _Class>
_NS_INLINE NS::SharedPtr<_Class>::operator const _Class*() const {
  return m_pObject;
}

template<class _Class>
_NS_INLINE NS::SharedPtr<_Class>::operator _Class*() const {
  return m_pObject;
}
...
_NS_INLINE bool operator==(
  const NS::SharedPtr<_ClassLhs>& lhs,
